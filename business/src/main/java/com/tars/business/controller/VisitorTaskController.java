...
import com.tars.business.service.task.VisitorTaskService;
...
@RestController
@RequestMapping("/business/visitor/task")
@CrossOrigin(origins = "*")
public class VisitorTaskController {
    @Autowired
    private VisitorTaskService myService;
    @Autowired
    ...
    public VisitorInfo sendSFS(VisitorInfo info) {
        // 模拟发送
        System.out.println("向司法所发送会客审批信息表");
    ...
    // 模拟向社区矫正机构发送
    public VisitorInfo sendJGSH(VisitorInfo ban) {
        return recvSFS(ban);
    }

    // 接收委托方司法所的审核意见
    public VisitorInfo recvSFS(VisitorInfo ban) {
    ...
    public VisitorInfo recvJZJG(VisitorInfo ban) {
        ban.setStep(ban.getStep() + 1);
        String processId = ban.getProcessId();
        // 社区矫正机构审批结束，得出结果
        complete(processId);
