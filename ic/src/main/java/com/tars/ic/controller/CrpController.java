...
public class CrpController {
  ...
  @Autowired
  private CrpInfoService service;
  @Autowired
  private CrpService peopleService;
  @Autowired
  ...
  public String getName(@PathVariable("dxbh") String dxbh) {
    CorrectionInfo dx = service.query().eq("dxbh", dxbh).one();
    return dx.getXm();
  }

  @GetMapping("/random")
  public String randomDxbh() {
    List<CorrectionInfo> list = service.list();
  ...
  public ResponseResult<String> export(
      @RequestBody CorrectionInfo crp) {
    try {
  ...
  @GetMapping("/all")
  public ResponseResult<List<CorrectionInfo>> getAll() {
    try {
  ...
  @GetMapping("/{dxbh}")
  public ResponseResult<CorrectionInfo> getCrp(
      @PathVariable("dxbh") String dxbh
  ) {
    CorrectionInfo temp = service.query().eq("dxbh", dxbh)
        .one();
    if (temp == null) {
   ...
   * 为入矫人员初始化出境信息
   */
  void initNoExit(CorrectionInfo crp) {
   ...
   * 为入矫人员初始化管理类别
   */
  void initCategory(CorrectionInfo crp) {
   ...
   * 为入矫人员初始化考核奖惩信息
   */
  void initAssessmentScore(CorrectionInfo crp) {
  ...
  public ResponseResult<Boolean> register(@RequestBody CorrectionInfo crp) {
    try {
      crp.setStatus(JZStatus.NOT.code)
          .setSfdcpg("0");
      CorrectionPeople people = CorrectionPeople.builder().dxbh(crp.getDxbh())
          .build();
  ...
  @PostMapping("/update")
  public ResponseResult<Boolean> update(@RequestBody CorrectionInfo crp) {
    try {
      crp.setGmt_modified(DateUtil.now());
